...
cmik_works <-function(X,Y,bw=c(0.8,0.8),kmax=floor(sqrt(length(X)))){
    ...
    for (i in 1:N){
        ...
        # Cound[tN[i]]t number within bandwidth
        s[i] <- sum(xdiffs[, i] < bw[1])
        # Get index of s[i]th neighbour
        # (Can be outside bandwidth!)
        # I.e., this is index of closest point
        ...
        kN[i] <- order(d)[k[i] + 1]
        eD[i] <- sort(d)[k[i] + 1] 
        # This will be either d[sN[i]] or d[tN[i]]
        # IF THERE IS NO KMAX
        pM[i] <- which.max(M[kN[i], ])
        l[i] <- sum(xdiffs[, i] < eD[i])
        m[i] <- sum(ydiffs[, i] < eD[i])
...
# Doesn't work, can't sort the diffs because d will be wrong.
cmik_exp <-function(X,Y,bw=c(0.8,0.8),kmax=floor(sqrt(length(X)))){
    ...
    for (i in 1:N){
        ...
        M <- cbind(xd, yd) # This can't be sorted
        # Max norm distance to every other point (X,Y pair)
        d <- apply(M, 1, max)
